// Kobalt Logo 3D ASCII Art Shader
// Renders the Kobalt icon as a 3D extruded model displayed with ASCII characters,
// slowly pivoting around the Y-axis. Place BEFORE tft.glsl in Ghostty config.
...
// Kobalt icon 2D SDF (from kobalt-logo.glsl)
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    // ── Rotation matrix ─────────────────────────────────────────────
    float angle = OSCILLATE
        ? sin(iTime * ROTATION_SPEED) * OSCILLATE_RANGE
