...
// with a dark blue animated border and a "spin-in" deceleration on tab open.
...
const float BORDER_WIDTH       = 0.02;                    // SDF units for border thickness
const float BORDER_SPEED       = 2.0;                     // vertical sweep speed
const float BORDER_PULSE_SPEED = 1.5;                     // pulsation frequency
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    if (mr.hit) {
        ...
        // Sharp border: only the outermost pixels, with thin anti-alias
